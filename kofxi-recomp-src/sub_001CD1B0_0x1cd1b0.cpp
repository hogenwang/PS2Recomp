...
void sub_001CD1B0_0x1cd1b0(uint8_t* rdram, R5900Context* ctx, PS2Runtime *runtime) {
    ...
    ctx->pc = 0x1CD1ECu;
    ctx->in_delay_slot = true; ctx->branch_pc = 0x1CD1E8u;
    ...
    if (runtime->hasFunction(0x1C25B8u)) {
        ...
        if (ctx->pc == __entryPc) { ctx->pc = 0x1CD1F0u; }
        if (ctx->pc != 0x1CD1F0u) { return; }
    } else {
        const uint32_t __entryPc = ctx->pc;
...
label_1cd1f0:
    ...
    ctx->pc = 0x1CD1FCu;
    {
        ...
        ctx->pc = 0x1CD200u;
        ctx->in_delay_slot = true; ctx->branch_pc = 0x1CD1FCu;
