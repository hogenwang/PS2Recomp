...
label_21b0c8:
    ...
    ctx->pc = 0x21B0D0u;
    {
        ...
        (void)runtime->dispatchGuestBranch(rdram, ctx, jumpTarget, 0x21B0D0u, 0u, PS2Runtime::GuestBranchKind::Return, "JR $ra");
        return;
        #else
        ctx->pc = jumpTarget;
        return;
...
label_21b100:
    ...
    ctx->pc = 0x21B114u;
    {
        ...
        ctx->pc = 0x21B118u;
        ctx->in_delay_slot = true;
        ctx->branch_pc = 0x21B114u;
        // 0x21b118: 0x27bd0020  addiu       $sp, $sp, 0x20 (Delay Slot)
        SET_GPR_S32(ctx, 29, (int32_t)ADD32(GPR_U32(ctx, 29), 32));
        ctx->in_delay_slot = false;
        ctx->pc = jumpTarget;
        #if defined(PS2X_STRICT_RETURN_DIAGNOSTICS) && PS2X_STRICT_RETURN_DIAGNOSTICS
        (void)runtime->dispatchGuestBranch(rdram, ctx, jumpTarget, 0x21B114u, 0u, PS2Runtime::GuestBranchKind::Return, "JR $ra");
        return;
        #else
        ctx->pc = jumpTarget;
        return;
    ...
    ctx->pc = 0x21b11cu;
    // NOP
    if (ctx->pc == 0x21b11cu) { ctx->pc = 0x21b120u; }
}
