...
void sub_002AFB78_0x2afb78(uint8_t* rdram, R5900Context* ctx, PS2Runtime *runtime) {
    ...
    ctx->pc = 0x2AFB84u;
    ctx->in_delay_slot = true; ctx->branch_pc = 0x2AFB80u;
    ...
    if (runtime->hasFunction(0x2AFB98u)) {
        ...
        if (ctx->pc == __entryPc) { ctx->pc = 0x2AFB88u; }
        if (ctx->pc != 0x2AFB88u) { return; }
    } else {
        const uint32_t __entryPc = ctx->pc;
...
label_2afb88:
    ...
    ctx->pc = 0x2AFB8Cu;
    {
        ...
        ctx->pc = 0x2AFB90u;
        ctx->in_delay_slot = true; ctx->branch_pc = 0x2AFB8Cu;
