...
void sub_0020C0F0_0x20c0f0(uint8_t* rdram, R5900Context* ctx, PS2Runtime *runtime) {
    ...
    ctx->pc = 0x20C100u;
    {
        ...
        ctx->branch_pc = 0x20C100u;
        // 0x20c104: 0xac64a458  sw          $a0, -0x5BA8($v1) (Delay Slot)
        WRITE32(ADD32(GPR_U32(ctx, 3), 4294943832), GPR_U32(ctx, 4));
        ctx->in_delay_slot = false;
        ctx->pc = jumpTarget;
        #if defined(PS2X_STRICT_RETURN_DIAGNOSTICS) && PS2X_STRICT_RETURN_DIAGNOSTICS
        (void)runtime->dispatchGuestBranch(rdram, ctx, jumpTarget, 0x20C100u, 0u, PS2Runtime::GuestBranchKind::Return, "JR $ra");
        return;
        #else
        ctx->pc = jumpTarget;
        return;
    ...
    ctx->pc = 0x20c10cu;
    // NOP
    if (ctx->pc == 0x20c10cu) { ctx->pc = 0x20c110u; }
}
