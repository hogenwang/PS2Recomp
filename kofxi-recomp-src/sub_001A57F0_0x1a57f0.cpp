...
label_1a5830:
    ...
    ctx->pc = 0x1A585Cu;
    {
        ...
        ctx->branch_pc = 0x1A585Cu;
        // 0x1a5860: 0xac600000  sw          $zero, 0x0($v1) (Delay Slot)
        WRITE32(ADD32(GPR_U32(ctx, 3), 0), GPR_U32(ctx, 0));
        ctx->in_delay_slot = false;
        ctx->pc = jumpTarget;
        #if defined(PS2X_STRICT_RETURN_DIAGNOSTICS) && PS2X_STRICT_RETURN_DIAGNOSTICS
        (void)runtime->dispatchGuestBranch(rdram, ctx, jumpTarget, 0x1A585Cu, 0u, PS2Runtime::GuestBranchKind::Return, "JR $ra");
        return;
        #else
        ctx->pc = jumpTarget;
        return;
    ...
    ctx->pc = 0x1a586cu;
    // NOP
    if (ctx->pc == 0x1a586cu) { ctx->pc = 0x1a5870u; }
}
