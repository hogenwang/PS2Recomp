#include <stdexcept>
#include "ps2_runtime_macros.h"
#include "ps2_runtime.h"
#include "ps2_recompiled_functions.h"
#include "ps2_recompiled_stubs.h"

#include "ps2_syscalls.h"
#include "ps2_stubs.h"

#ifdef PS2_FUNCTION_LOG_TRACKER
#include "ps2_log.h"
#endif

// Function: sub_0032D170
// Address: 0x32d170 - 0x32d1d0
void sub_0032D170_0x32d170(uint8_t* rdram, R5900Context* ctx, PS2Runtime *runtime) {
#ifdef PS2_FUNCTION_LOG_TRACKER
    PS_LOG_ENTRY("sub_0032D170_0x32d170");
#endif

    switch (ctx->pc) {
        case 0x32d1a0u: goto label_32d1a0;
        case 0x32d1b4u: goto label_32d1b4;
        default: break;
    }

    ctx->pc = 0x32d170u;

    // 0x32d170: 0x27bdfff0  addiu       $sp, $sp, -0x10
    ctx->pc = 0x32d170u;
    SET_GPR_S32(ctx, 29, (int32_t)ADD32(GPR_U32(ctx, 29), 4294967280));
    // 0x32d174: 0x3c020093  lui         $v0, 0x93
    ctx->pc = 0x32d174u;
    SET_GPR_S32(ctx, 2, (int32_t)((uint32_t)147 << 16));
    // 0x32d178: 0xffbf0000  sd          $ra, 0x0($sp)
    ctx->pc = 0x32d178u;
    WRITE64(ADD32(GPR_U32(ctx, 29), 0), GPR_U64(ctx, 31));
    // 0x32d17c: 0x3c050040  lui         $a1, 0x40
    ctx->pc = 0x32d17cu;
    SET_GPR_S32(ctx, 5, (int32_t)((uint32_t)64 << 16));
    // 0x32d180: 0x9044f35b  lbu         $a0, -0xCA5($v0)
    ctx->pc = 0x32d180u;
    SET_GPR_U32(ctx, 4, (uint8_t)READ8(ADD32(GPR_U32(ctx, 2), 4294964059)));
    // 0x32d184: 0x41880  sll         $v1, $a0, 2
    ctx->pc = 0x32d184u;
    SET_GPR_S32(ctx, 3, (int32_t)SLL32(GPR_U32(ctx, 4), 2));
    // 0x32d188: 0x3c02003d  lui         $v0, 0x3D
    ctx->pc = 0x32d188u;
    SET_GPR_S32(ctx, 2, (int32_t)((uint32_t)61 << 16));
    // 0x32d18c: 0x641821  addu        $v1, $v1, $a0
    ctx->pc = 0x32d18cu;
    SET_GPR_S32(ctx, 3, (int32_t)ADD32(GPR_U32(ctx, 3), GPR_U32(ctx, 4)));
    // 0x32d190: 0x24421820  addiu       $v0, $v0, 0x1820
    ctx->pc = 0x32d190u;
    SET_GPR_S32(ctx, 2, (int32_t)ADD32(GPR_U32(ctx, 2), 6176));
    // 0x32d194: 0x31880  sll         $v1, $v1, 2
    ctx->pc = 0x32d194u;
    SET_GPR_S32(ctx, 3, (int32_t)SLL32(GPR_U32(ctx, 3), 2));
    // 0x32d198: 0xc0cb3e4  jal         func_32CF90
    ctx->pc = 0x32D198u;
    SET_GPR_U32(ctx, 31, 0x32D1A0u);
    ctx->pc = 0x32D19Cu;
    ctx->in_delay_slot = true;
    ctx->branch_pc = 0x32D198u;
    // 0x32d19c: 0x432021  addu        $a0, $v0, $v1 (Delay Slot)
    SET_GPR_S32(ctx, 4, (int32_t)ADD32(GPR_U32(ctx, 2), GPR_U32(ctx, 3)));
    ctx->in_delay_slot = false;
    ctx->pc = 0x32CF90u;
    if (!runtime->dispatchGuestBranch(rdram, ctx, 0x32CF90u, 0x32D198u, 0x32D1A0u, PS2Runtime::GuestBranchKind::DirectCall, "JAL")) {
        return;
    }
    ctx->pc = 0x32D1A0u;
label_32d1a0:
    // 0x32d1a0: 0x3c0301dd  lui         $v1, 0x1DD
    ctx->pc = 0x32d1a0u;
    SET_GPR_S32(ctx, 3, (int32_t)((uint32_t)477 << 16));
    // 0x32d1a4: 0x24040005  addiu       $a0, $zero, 0x5
    ctx->pc = 0x32d1a4u;
    SET_GPR_S32(ctx, 4, (int32_t)ADD32(GPR_U32(ctx, 0), 5));
    // 0x32d1a8: 0xac62f368  sw          $v0, -0xC98($v1)
    ctx->pc = 0x32d1a8u;
    WRITE32(ADD32(GPR_U32(ctx, 3), 4294964072), GPR_U32(ctx, 2));
    // 0x32d1ac: 0xc0cb224  jal         func_32C890
    ctx->pc = 0x32D1ACu;
    SET_GPR_U32(ctx, 31, 0x32D1B4u);
    ctx->pc = 0x32D1B0u;
    ctx->in_delay_slot = true;
    ctx->branch_pc = 0x32D1ACu;
    // 0x32d1b0: 0x40282d  daddu       $a1, $v0, $zero (Delay Slot)
    SET_GPR_U64(ctx, 5, (uint64_t)GPR_U64(ctx, 2) + (uint64_t)GPR_U64(ctx, 0));
    ctx->in_delay_slot = false;
    ctx->pc = 0x32C890u;
    if (!runtime->dispatchGuestBranch(rdram, ctx, 0x32C890u, 0x32D1ACu, 0x32D1B4u, PS2Runtime::GuestBranchKind::DirectCall, "JAL")) {
        return;
    }
    ctx->pc = 0x32D1B4u;
label_32d1b4:
    // 0x32d1b4: 0xdfbf0000  ld          $ra, 0x0($sp)
    ctx->pc = 0x32d1b4u;
    SET_GPR_U64(ctx, 31, READ64(ADD32(GPR_U32(ctx, 29), 0)));
    // 0x32d1b8: 0x24020001  addiu       $v0, $zero, 0x1
    ctx->pc = 0x32d1b8u;
    SET_GPR_S32(ctx, 2, (int32_t)ADD32(GPR_U32(ctx, 0), 1));
    // 0x32d1bc: 0x3e00008  jr          $ra
    ctx->pc = 0x32D1BCu;
    {
        const uint32_t jumpTarget = GPR_U32(ctx, 31);
        ctx->pc = 0x32D1C0u;
        ctx->in_delay_slot = true;
        ctx->branch_pc = 0x32D1BCu;
        // 0x32d1c0: 0x27bd0010  addiu       $sp, $sp, 0x10 (Delay Slot)
        SET_GPR_S32(ctx, 29, (int32_t)ADD32(GPR_U32(ctx, 29), 16));
        ctx->in_delay_slot = false;
        ctx->pc = jumpTarget;
        #if defined(PS2X_STRICT_RETURN_DIAGNOSTICS) && PS2X_STRICT_RETURN_DIAGNOSTICS
        (void)runtime->dispatchGuestBranch(rdram, ctx, jumpTarget, 0x32D1BCu, 0u, PS2Runtime::GuestBranchKind::Return, "JR $ra");
        return;
        #else
        ctx->pc = jumpTarget;
        return;
        #endif
    }
    ctx->pc = 0x32D1C4u;
    // 0x32d1c4: 0x0  nop
    ctx->pc = 0x32d1c4u;
    // NOP
    // 0x32d1c8: 0x0  nop
    ctx->pc = 0x32d1c8u;
    // NOP
    // 0x32d1cc: 0x0  nop
    ctx->pc = 0x32d1ccu;
    // NOP
    if (ctx->pc == 0x32d1ccu) { ctx->pc = 0x32d1d0u; }
}
