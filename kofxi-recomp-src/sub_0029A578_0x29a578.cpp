...
void sub_0029A578_0x29a578(uint8_t* rdram, R5900Context* ctx, PS2Runtime *runtime) {
    ...
    ctx->pc = 0x29A584u;
    {
        ...
        ctx->branch_pc = 0x29A584u;
        // 0x29a588: 0xac6467e4  sw          $a0, 0x67E4($v1) (Delay Slot)
        WRITE32(ADD32(GPR_U32(ctx, 3), 26596), GPR_U32(ctx, 4));
        ctx->in_delay_slot = false;
        ctx->pc = jumpTarget;
        #if defined(PS2X_STRICT_RETURN_DIAGNOSTICS) && PS2X_STRICT_RETURN_DIAGNOSTICS
        (void)runtime->dispatchGuestBranch(rdram, ctx, jumpTarget, 0x29A584u, 0u, PS2Runtime::GuestBranchKind::Return, "JR $ra");
        return;
        #else
        ctx->pc = jumpTarget;
        return;
    ...
    ctx->pc = 0x29a58cu;
    // NOP
    if (ctx->pc == 0x29a58cu) { ctx->pc = 0x29a590u; }
}
