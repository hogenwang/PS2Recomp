...
void sub_001B1A70_0x1b1a70(uint8_t* rdram, R5900Context* ctx, PS2Runtime *runtime) {
    ...
    ctx->branch_pc = 0x1B1A7Cu;
    // 0x1b1a80: 0x27bd0010  addiu       $sp, $sp, 0x10 (Delay Slot)
    SET_GPR_S32(ctx, 29, (int32_t)ADD32(GPR_U32(ctx, 29), 16));
    ctx->in_delay_slot = false;
    ctx->pc = 0x1B7BA0u;
    if (!runtime->dispatchGuestBranch(rdram, ctx, 0x1B7BA0u, 0x1B1A7Cu, 0x0u, PS2Runtime::GuestBranchKind::DirectJump, "J")) {
        return;
    }
    ctx->pc = 0x1B1A84u;
    // 0x1b1a84: 0x0  nop
    ctx->pc = 0x1b1a84u;
    // NOP
    if (ctx->pc == 0x1b1a84u) { ctx->pc = 0x1b1a88u; }
}
