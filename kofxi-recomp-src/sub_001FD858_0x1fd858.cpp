...
label_1fd878:
    ...
    ctx->pc = 0x1FD8ACu;
    {
        ...
        ctx->pc = 0x1FD8B0u;
        ctx->in_delay_slot = true;
        ctx->branch_pc = 0x1FD8ACu;
        // 0x1fd8b0: 0xac8513b4  sw          $a1, 0x13B4($a0) (Delay Slot)
        WRITE32(ADD32(GPR_U32(ctx, 4), 5044), GPR_U32(ctx, 5));
        ctx->in_delay_slot = false;
        ctx->pc = jumpTarget;
        #if defined(PS2X_STRICT_RETURN_DIAGNOSTICS) && PS2X_STRICT_RETURN_DIAGNOSTICS
        (void)runtime->dispatchGuestBranch(rdram, ctx, jumpTarget, 0x1FD8ACu, 0u, PS2Runtime::GuestBranchKind::Return, "JR $ra");
        return;
        #else
        ctx->pc = jumpTarget;
        return;
    ...
    ctx->pc = 0x1fd8b4u;
    // NOP
    if (ctx->pc == 0x1fd8b4u) { ctx->pc = 0x1fd8b8u; }
}
