...
void sub_002A96E0_0x2a96e0(uint8_t* rdram, R5900Context* ctx, PS2Runtime *runtime) {
    ...
    ctx->pc = 0x2A96E4u;
    {
        ...
        ctx->branch_pc = 0x2A96E4u;
        // 0x2a96e8: 0xac446aa8  sw          $a0, 0x6AA8($v0) (Delay Slot)
        WRITE32(ADD32(GPR_U32(ctx, 2), 27304), GPR_U32(ctx, 4));
        ctx->in_delay_slot = false;
        ctx->pc = jumpTarget;
        #if defined(PS2X_STRICT_RETURN_DIAGNOSTICS) && PS2X_STRICT_RETURN_DIAGNOSTICS
        (void)runtime->dispatchGuestBranch(rdram, ctx, jumpTarget, 0x2A96E4u, 0u, PS2Runtime::GuestBranchKind::Return, "JR $ra");
        return;
        #else
        ctx->pc = jumpTarget;
        return;
    ...
    ctx->pc = 0x2a96ecu;
    // NOP
    if (ctx->pc == 0x2a96ecu) { ctx->pc = 0x2a96f0u; }
}
