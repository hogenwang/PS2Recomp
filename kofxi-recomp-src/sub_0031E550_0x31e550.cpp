...
void sub_0031E550_0x31e550(uint8_t* rdram, R5900Context* ctx, PS2Runtime *runtime) {
    ...
    ctx->pc = 0x31E57Cu;
    {
        ...
        ctx->branch_pc = 0x31E57Cu;
        // 0x31e580: 0xac604780  sw          $zero, 0x4780($v1) (Delay Slot)
        WRITE32(ADD32(GPR_U32(ctx, 3), 18304), GPR_U32(ctx, 0));
        ctx->in_delay_slot = false;
        ctx->pc = jumpTarget;
        #if defined(PS2X_STRICT_RETURN_DIAGNOSTICS) && PS2X_STRICT_RETURN_DIAGNOSTICS
        (void)runtime->dispatchGuestBranch(rdram, ctx, jumpTarget, 0x31E57Cu, 0u, PS2Runtime::GuestBranchKind::Return, "JR $ra");
        return;
        #else
        ctx->pc = jumpTarget;
        return;
    ...
    ctx->pc = 0x31e58cu;
    // NOP
    if (ctx->pc == 0x31e58cu) { ctx->pc = 0x31e590u; }
}
