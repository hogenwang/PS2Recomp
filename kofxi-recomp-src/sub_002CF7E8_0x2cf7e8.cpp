...
void sub_002CF7E8_0x2cf7e8(uint8_t* rdram, R5900Context* ctx, PS2Runtime *runtime) {
    ...
    ctx->pc = 0x2CF80Cu;
    ctx->in_delay_slot = true; ctx->branch_pc = 0x2CF808u;
    ...
    if (runtime->hasFunction(0x2D82E0u)) {
        ...
        if (ctx->pc == __entryPc) { ctx->pc = 0x2CF810u; }
        if (ctx->pc != 0x2CF810u) { return; }
    } else {
        const uint32_t __entryPc = ctx->pc;
...
label_2cf810:
    ...
    ctx->pc = 0x2CF83Cu;
    ctx->pc = 0x2CF840u;
    ctx->in_delay_slot = true; ctx->branch_pc = 0x2CF83Cu;
