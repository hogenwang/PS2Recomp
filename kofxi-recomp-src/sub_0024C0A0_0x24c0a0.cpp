...
void sub_0024C0A0_0x24c0a0(uint8_t* rdram, R5900Context* ctx, PS2Runtime *runtime) {
    ...
    ctx->pc = 0x24C0CCu;
    ctx->in_delay_slot = true; ctx->branch_pc = 0x24C0C8u;
    ...
    if (runtime->hasFunction(0x123258u)) {
        ...
        if (ctx->pc == __entryPc) { ctx->pc = 0x24C0D0u; }
        if (ctx->pc != 0x24C0D0u) { return; }
    } else {
        const uint32_t __entryPc = ctx->pc;
...
label_24c0d0:
    ...
    ctx->pc = 0x24C104u;
    {
        ...
        ctx->pc = 0x24C108u;
        ctx->in_delay_slot = true; ctx->branch_pc = 0x24C104u;
