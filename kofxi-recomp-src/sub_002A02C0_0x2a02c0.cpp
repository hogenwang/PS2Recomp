...
void sub_002A02C0_0x2a02c0(uint8_t* rdram, R5900Context* ctx, PS2Runtime *runtime) {
    ...
    if (runtime->hasFunction(0x2A2FD8u)) {
        ...
        if (ctx->pc == __entryPc) { ctx->pc = 0x2A02D0u; }
        if (ctx->pc != 0x2A02D0u) { return; }
    } else {
        const uint32_t __entryPc = ctx->pc;
...
label_2a02d0:
    ...
    ctx->pc = 0x2A02D4u;
    {
        ...
        ctx->pc = 0x2A02D8u;
        ctx->in_delay_slot = true; ctx->branch_pc = 0x2A02D4u;
...
label_2a02fc:
    ...
    ctx->pc = 0x2A0300u;
    {
        ...
        ctx->pc = 0x2A0304u;
        ctx->in_delay_slot = true; ctx->branch_pc = 0x2A0300u;
