...
label_2a05dc:
    ...
    ctx->pc = 0x2A05D8u;
    {
        ...
        ctx->pc = 0x2A05DCu;
        ctx->in_delay_slot = true; ctx->branch_pc = 0x2A05D8u;
            // 0x2a05dc: 0x24050008  addiu       $a1, $zero, 0x8 (Delay Slot)
        SET_GPR_S32(ctx, 5, (int32_t)ADD32(GPR_U32(ctx, 0), 8));
        ...
        ctx->pc = jumpTarget;
        {
            ...
            if (ctx->pc == __entryPc) { ctx->pc = 0x2A05E0u; }
            if (ctx->pc != 0x2A05E0u) { return; }
        }
        }
    }
    ctx->pc = 0x2A05E0u;
...
label_2a05e8:
    ...
    ctx->pc = 0x2A05E4u;
    {
        ...
        ctx->pc = 0x2A05E8u;
        ctx->in_delay_slot = true; ctx->branch_pc = 0x2A05E4u;
            // 0x2a05e8: 0x27bd0010  addiu       $sp, $sp, 0x10 (Delay Slot)
        SET_GPR_S32(ctx, 29, (int32_t)ADD32(GPR_U32(ctx, 29), 16));
        ctx->in_delay_slot = false;
        ctx->pc = jumpTarget;
