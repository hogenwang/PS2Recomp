...
void sub_001F4238_0x1f4238(uint8_t* rdram, R5900Context* ctx, PS2Runtime *runtime) {
    ...
    ctx->branch_pc = 0x1F4244u;
    // 0x1f4248: 0x8c852020  lw          $a1, 0x2020($a0) (Delay Slot)
    SET_GPR_S32(ctx, 5, (int32_t)READ32(ADD32(GPR_U32(ctx, 4), 8224)));
    ctx->in_delay_slot = false;
    ctx->pc = 0x1EA980u;
    if (!runtime->dispatchGuestBranch(rdram, ctx, 0x1EA980u, 0x1F4244u, 0x1F424Cu, PS2Runtime::GuestBranchKind::DirectCall, "JAL")) {
        return;
    }
    ctx->pc = 0x1F424Cu;
label_1f424c:
    ...
    ctx->pc = 0x1F4250u;
    {
        ...
        ctx->pc = 0x1F4254u;
        ctx->in_delay_slot = true;
        ctx->branch_pc = 0x1F4250u;
        // 0x1f4254: 0x27bd0010  addiu       $sp, $sp, 0x10 (Delay Slot)
        SET_GPR_S32(ctx, 29, (int32_t)ADD32(GPR_U32(ctx, 29), 16));
        ctx->in_delay_slot = false;
        ctx->pc = jumpTarget;
        #if defined(PS2X_STRICT_RETURN_DIAGNOSTICS) && PS2X_STRICT_RETURN_DIAGNOSTICS
        (void)runtime->dispatchGuestBranch(rdram, ctx, jumpTarget, 0x1F4250u, 0u, PS2Runtime::GuestBranchKind::Return, "JR $ra");
        return;
        #else
        ctx->pc = jumpTarget;
        return;
