...
void sub_002CF728_0x2cf728(uint8_t* rdram, R5900Context* ctx, PS2Runtime *runtime) {
    ...
    ctx->pc = 0x2CF738u;
    ctx->in_delay_slot = true; ctx->branch_pc = 0x2CF734u;
    ...
    if (runtime->hasFunction(0x2CF750u)) {
        ...
        if (ctx->pc == __entryPc) { ctx->pc = 0x2CF73Cu; }
        if (ctx->pc != 0x2CF73Cu) { return; }
    } else {
        const uint32_t __entryPc = ctx->pc;
...
label_2cf73c:
    ...
    ctx->pc = 0x2CF748u;
    ctx->pc = 0x2CF74Cu;
    ctx->in_delay_slot = true; ctx->branch_pc = 0x2CF748u;
