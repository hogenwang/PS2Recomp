...
void sub_001D6568_0x1d6568(uint8_t* rdram, R5900Context* ctx, PS2Runtime *runtime) {
    ...
    ctx->pc = 0x1D6588u;
    ctx->in_delay_slot = true; ctx->branch_pc = 0x1D6584u;
    ...
    if (runtime->hasFunction(0x1D88E8u)) {
        ...
        if (ctx->pc == __entryPc) { ctx->pc = 0x1D658Cu; }
        if (ctx->pc != 0x1D658Cu) { return; }
    } else {
        const uint32_t __entryPc = ctx->pc;
...
label_1d65a0:
    ...
    ctx->pc = 0x1D65ACu;
    {
        ...
        ctx->pc = 0x1D65B0u;
        ctx->in_delay_slot = true; ctx->branch_pc = 0x1D65ACu;
