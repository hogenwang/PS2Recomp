...
label_1df704:
    ...
    ctx->pc = 0x1DF704u;
    {
        ...
        ctx->pc = 0x1DF708u;
        ctx->in_delay_slot = true;
        ctx->branch_pc = 0x1DF704u;
        // 0x1df708: 0x60102d  daddu       $v0, $v1, $zero (Delay Slot)
        SET_GPR_U64(ctx, 2, (uint64_t)GPR_U64(ctx, 3) + (uint64_t)GPR_U64(ctx, 0));
        ctx->in_delay_slot = false;
        ctx->pc = jumpTarget;
        #if defined(PS2X_STRICT_RETURN_DIAGNOSTICS) && PS2X_STRICT_RETURN_DIAGNOSTICS
        (void)runtime->dispatchGuestBranch(rdram, ctx, jumpTarget, 0x1DF704u, 0u, PS2Runtime::GuestBranchKind::Return, "JR $ra");
        return;
        #else
        ctx->pc = jumpTarget;
        return;
    ...
    ctx->pc = 0x1df70cu;
    // NOP
    if (ctx->pc == 0x1df70cu) { ctx->pc = 0x1df710u; }
}
