...
label_1beb0c:
    ...
    ctx->pc = 0x1BEB08u;
    {
        ...
        ctx->in_delay_slot = false;
        ctx->pc = jumpTarget;
        if (!runtime->dispatchGuestBranch(rdram, ctx, jumpTarget, 0x1BEB08u, 0x1BEB10u, PS2Runtime::GuestBranchKind::IndirectCall, "JALR")) {
            return;
        }
    }
    ctx->pc = 0x1BEB10u;
...
label_1beb3c:
    ...
    ctx->pc = 0x1beb3cu;
    // NOP
    if (ctx->pc == 0x1beb3cu) { ctx->pc = 0x1beb40u; }
}
