...
void sub_003122B0_0x3122b0(uint8_t* rdram, R5900Context* ctx, PS2Runtime *runtime) {
    ...
    ctx->pc = 0x3122D8u;
    ctx->in_delay_slot = true;
    ctx->branch_pc = 0x3122D4u;
    // 0x3122d8: 0x9064d940  lbu         $a0, -0x26C0($v1) (Delay Slot)
    SET_GPR_U32(ctx, 4, (uint8_t)READ8(ADD32(GPR_U32(ctx, 3), 4294957376)));
    ctx->in_delay_slot = false;
    ctx->pc = 0x311F50u;
    if (!runtime->dispatchGuestBranch(rdram, ctx, 0x311F50u, 0x3122D4u, 0x3122DCu, PS2Runtime::GuestBranchKind::DirectCall, "JAL")) {
        return;
    }
    ctx->pc = 0x3122DCu;
label_3122dc:
...
label_3122e8:
    ...
    ctx->pc = 0x3122E8u;
    {
        ...
        (void)runtime->dispatchGuestBranch(rdram, ctx, jumpTarget, 0x3122E8u, 0u, PS2Runtime::GuestBranchKind::Return, "JR $ra");
        return;
        #else
        ctx->pc = jumpTarget;
        return;
