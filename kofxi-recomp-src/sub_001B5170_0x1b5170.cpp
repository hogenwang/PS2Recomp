...
void sub_001B5170_0x1b5170(uint8_t* rdram, R5900Context* ctx, PS2Runtime *runtime) {
    ...
    ctx->pc = 0x1A99D0u;
    if (runtime->hasFunction(0x1A99D0u)) {
        auto targetFn = runtime->lookupFunction(0x1A99D0u);
        ...
        const uint32_t __entryPc = ctx->pc;
        sub_001A99D0_0x1a99d0(rdram, ctx, runtime);
        if (ctx->pc == __entryPc) { ctx->pc = 0x1B5194u; }
        if (ctx->pc != 0x1B5194u) { return; }
    }
    ctx->pc = 0x1B5194u;
label_1b5194:
    ...
    ctx->pc = 0x1B51A8u;
    ctx->pc = 0x1B51ACu;
    ctx->in_delay_slot = true; ctx->branch_pc = 0x1B51A8u;
            // 0x1b51ac: 0x27bd0020  addiu       $sp, $sp, 0x20 (Delay Slot)
        SET_GPR_S32(ctx, 29, (int32_t)ADD32(GPR_U32(ctx, 29), 32));
        ctx->in_delay_slot = false;
    ctx->pc = 0x1A97E8u;
    {
        auto targetFn = runtime->lookupFunction(0x1A97E8u);
        const uint32_t __entryPc = ctx->pc;
        targetFn(rdram, ctx, runtime);
        return;
    }
