...
void sub_0030B940_0x30b940(uint8_t* rdram, R5900Context* ctx, PS2Runtime *runtime) {
    ...
    if (runtime->hasFunction(0x103AC8u)) {
        ...
        if (ctx->pc == __entryPc) { ctx->pc = 0x30B958u; }
        if (ctx->pc != 0x30B958u) { return; }
    } else {
        const uint32_t __entryPc = ctx->pc;
...
label_30b958:
    ...
    ctx->in_delay_slot = true; ctx->branch_pc = 0x30B960u;
        ...
        ctx->in_delay_slot = false;
    ctx->pc = 0x103740u;
    if (runtime->hasFunction(0x103740u)) {
        auto targetFn = runtime->lookupFunction(0x103740u);
        const uint32_t __entryPc = ctx->pc;
        targetFn(rdram, ctx, runtime);
        if (ctx->pc == __entryPc) { ctx->pc = 0x30B968u; }
        if (ctx->pc != 0x30B968u) { return; }
    } else {
        const uint32_t __entryPc = ctx->pc;
        sub_00103740_0x103740(rdram, ctx, runtime);
        if (ctx->pc == __entryPc) { ctx->pc = 0x30B968u; }
        if (ctx->pc != 0x30B968u) { return; }
    }
    ctx->pc = 0x30B968u;
label_30b968:
    ...
    ctx->pc = 0x30B970u;
    {
        ...
        ctx->pc = 0x30B974u;
        ctx->in_delay_slot = true; ctx->branch_pc = 0x30B970u;
