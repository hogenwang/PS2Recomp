...
label_2dccf4:
    ...
    ctx->pc = 0x2DCCF4u;
    {
        ...
        ctx->pc = 0x2DCCF8u;
        ctx->in_delay_slot = true;
        ctx->branch_pc = 0x2DCCF4u;
        // 0x2dccf8: 0x831023  subu        $v0, $a0, $v1 (Delay Slot)
        SET_GPR_S32(ctx, 2, (int32_t)SUB32(GPR_U32(ctx, 4), GPR_U32(ctx, 3)));
        ctx->in_delay_slot = false;
        ctx->pc = jumpTarget;
        #if defined(PS2X_STRICT_RETURN_DIAGNOSTICS) && PS2X_STRICT_RETURN_DIAGNOSTICS
        (void)runtime->dispatchGuestBranch(rdram, ctx, jumpTarget, 0x2DCCF4u, 0u, PS2Runtime::GuestBranchKind::Return, "JR $ra");
        return;
        #else
        ctx->pc = jumpTarget;
        return;
    ...
    ctx->pc = 0x2dccfcu;
    // NOP
    if (ctx->pc == 0x2dccfcu) { ctx->pc = 0x2dcd00u; }
}
