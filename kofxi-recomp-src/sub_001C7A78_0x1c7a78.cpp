...
void sub_001C7A78_0x1c7a78(uint8_t* rdram, R5900Context* ctx, PS2Runtime *runtime) {
    ...
    ctx->pc = 0x1C7A84u;
    {
        ...
        ctx->branch_pc = 0x1C7A84u;
        // 0x1c7a88: 0xac654370  sw          $a1, 0x4370($v1) (Delay Slot)
        WRITE32(ADD32(GPR_U32(ctx, 3), 17264), GPR_U32(ctx, 5));
        ctx->in_delay_slot = false;
        ctx->pc = jumpTarget;
        #if defined(PS2X_STRICT_RETURN_DIAGNOSTICS) && PS2X_STRICT_RETURN_DIAGNOSTICS
        (void)runtime->dispatchGuestBranch(rdram, ctx, jumpTarget, 0x1C7A84u, 0u, PS2Runtime::GuestBranchKind::Return, "JR $ra");
        return;
        #else
        ctx->pc = jumpTarget;
        return;
    ...
    ctx->pc = 0x1c7a8cu;
    // NOP
    if (ctx->pc == 0x1c7a8cu) { ctx->pc = 0x1c7a90u; }
}
