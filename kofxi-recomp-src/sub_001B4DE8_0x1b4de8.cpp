#include "ps2_runtime_macros.h"
#include "ps2_runtime.h"
#include "ps2_recompiled_functions.h"
#include "ps2_recompiled_stubs.h"

#include "ps2_syscalls.h"
#include "ps2_stubs.h"

#ifdef PS2_FUNCTION_LOG_TRACKER
#include "ps2_log.h"
#endif

// Function: sub_001B4DE8
// Address: 0x1b4de8 - 0x1b4e08
void sub_001B4DE8_0x1b4de8(uint8_t* rdram, R5900Context* ctx, PS2Runtime *runtime) {
#ifdef PS2_FUNCTION_LOG_TRACKER
    PS_LOG_ENTRY("sub_001B4DE8_0x1b4de8");
#endif

    switch (ctx->pc) {
        case 0x1b4df8u: goto label_1b4df8;
        default: break;
    }

    ctx->pc = 0x1b4de8u;

    // 0x1b4de8: 0x27bdfff0  addiu       $sp, $sp, -0x10
    ctx->pc = 0x1b4de8u;
    SET_GPR_S32(ctx, 29, (int32_t)ADD32(GPR_U32(ctx, 29), 4294967280));
    // 0x1b4dec: 0xffbf0000  sd          $ra, 0x0($sp)
    ctx->pc = 0x1b4decu;
    WRITE64(ADD32(GPR_U32(ctx, 29), 0), GPR_U64(ctx, 31));
    // 0x1b4df0: 0xc07299a  jal         func_1CA668
    ctx->pc = 0x1B4DF0u;
    SET_GPR_U32(ctx, 31, 0x1B4DF8u);
    ctx->pc = 0x1CA668u;
    if (runtime->hasFunction(0x1CA668u)) {
        auto targetFn = runtime->lookupFunction(0x1CA668u);
        const uint32_t __entryPc = ctx->pc;
        targetFn(rdram, ctx, runtime);
        if (ctx->pc == __entryPc) { ctx->pc = 0x1B4DF8u; }
        if (ctx->pc != 0x1B4DF8u) { return; }
    } else {
        const uint32_t __entryPc = ctx->pc;
        sub_001CA668_0x1ca668(rdram, ctx, runtime);
        if (ctx->pc == __entryPc) { ctx->pc = 0x1B4DF8u; }
        if (ctx->pc != 0x1B4DF8u) { return; }
    }
    ctx->pc = 0x1B4DF8u;
label_1b4df8:
    // 0x1b4df8: 0xdfbf0000  ld          $ra, 0x0($sp)
    ctx->pc = 0x1b4df8u;
    SET_GPR_U64(ctx, 31, READ64(ADD32(GPR_U32(ctx, 29), 0)));
    // 0x1b4dfc: 0x102d  daddu       $v0, $zero, $zero
    ctx->pc = 0x1b4dfcu;
    SET_GPR_U64(ctx, 2, (uint64_t)GPR_U64(ctx, 0) + (uint64_t)GPR_U64(ctx, 0));
    // 0x1b4e00: 0x3e00008  jr          $ra
    ctx->pc = 0x1B4E00u;
    {
        uint32_t jumpTarget = GPR_U32(ctx, 31);
        ctx->pc = 0x1B4E04u;
        ctx->in_delay_slot = true; ctx->branch_pc = 0x1B4E00u;
            // 0x1b4e04: 0x27bd0010  addiu       $sp, $sp, 0x10 (Delay Slot)
        SET_GPR_S32(ctx, 29, (int32_t)ADD32(GPR_U32(ctx, 29), 16));
        ctx->in_delay_slot = false;
        ctx->pc = jumpTarget;
        return;
    }
    ctx->pc = 0x1B4E08u;
    ctx->pc = 0x1b4e08u;
}
