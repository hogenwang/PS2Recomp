...
void sub_001E8BC8_0x1e8bc8(uint8_t* rdram, R5900Context* ctx, PS2Runtime *runtime) {
    ...
    ctx->pc = 0x1E8BD8u;
    ctx->in_delay_slot = true; ctx->branch_pc = 0x1E8BD4u;
    ...
    if (runtime->hasFunction(0x1B9370u)) {
        ...
        if (ctx->pc == __entryPc) { ctx->pc = 0x1E8BDCu; }
        if (ctx->pc != 0x1E8BDCu) { return; }
    } else {
        const uint32_t __entryPc = ctx->pc;
...
label_1e8bdc:
    ...
    ctx->pc = 0x1E8BE8u;
    {
        ...
        ctx->pc = 0x1E8BECu;
        ctx->in_delay_slot = true; ctx->branch_pc = 0x1E8BE8u;
