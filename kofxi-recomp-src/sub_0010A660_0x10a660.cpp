...
void sub_0010A660_0x10a660(uint8_t* rdram, R5900Context* ctx, PS2Runtime *runtime) {
    ...
    ctx->pc = 0x10A694u;
    ctx->in_delay_slot = true; ctx->branch_pc = 0x10A690u;
    ...
    if (runtime->hasFunction(0x109D78u)) {
        ...
        if (ctx->pc == __entryPc) { ctx->pc = 0x10A698u; }
        if (ctx->pc != 0x10A698u) { return; }
    } else {
        const uint32_t __entryPc = ctx->pc;
...
label_10a698:
    ...
    ctx->pc = 0x10A69Cu;
    {
        ...
        ctx->pc = 0x10A6A0u;
        ctx->in_delay_slot = true; ctx->branch_pc = 0x10A69Cu;
