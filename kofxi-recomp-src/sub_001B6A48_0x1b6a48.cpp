...
void sub_001B6A48_0x1b6a48(uint8_t* rdram, R5900Context* ctx, PS2Runtime *runtime) {
    ...
    ctx->branch_pc = 0x1B6A74u;
    // 0x1b6a78: 0xc0a02d  daddu       $s4, $a2, $zero (Delay Slot)
    SET_GPR_U64(ctx, 20, (uint64_t)GPR_U64(ctx, 6) + (uint64_t)GPR_U64(ctx, 0));
    ctx->in_delay_slot = false;
    ctx->pc = 0x1B8108u;
    if (!runtime->dispatchGuestBranch(rdram, ctx, 0x1B8108u, 0x1B6A74u, 0x1B6A7Cu, PS2Runtime::GuestBranchKind::DirectCall, "JAL")) {
        return;
    }
    ctx->pc = 0x1B6A7Cu;
label_1b6a7c:
    ...
    ctx->pc = 0x1b6abcu;
    // NOP
    if (ctx->pc == 0x1b6abcu) { ctx->pc = 0x1b6ac0u; }
}
