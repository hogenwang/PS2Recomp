...
void sub_0020A3E0_0x20a3e0(uint8_t* rdram, R5900Context* ctx, PS2Runtime *runtime) {
    ...
    ctx->pc = 0x20A3F0u;
    {
        ...
        ctx->branch_pc = 0x20A3F0u;
        // 0x20a3f4: 0xac64a060  sw          $a0, -0x5FA0($v1) (Delay Slot)
        WRITE32(ADD32(GPR_U32(ctx, 3), 4294942816), GPR_U32(ctx, 4));
        ctx->in_delay_slot = false;
        ctx->pc = jumpTarget;
        #if defined(PS2X_STRICT_RETURN_DIAGNOSTICS) && PS2X_STRICT_RETURN_DIAGNOSTICS
        (void)runtime->dispatchGuestBranch(rdram, ctx, jumpTarget, 0x20A3F0u, 0u, PS2Runtime::GuestBranchKind::Return, "JR $ra");
        return;
        #else
        ctx->pc = jumpTarget;
        return;
    ...
    ctx->pc = 0x20a3fcu;
    // NOP
    if (ctx->pc == 0x20a3fcu) { ctx->pc = 0x20a400u; }
}
