...
label_28ebb0:
    ...
    ctx->pc = 0x28EBB0u;
    ctx->in_delay_slot = true; ctx->branch_pc = 0x28EBACu;
            // 0x28ebb0: 0x402d  daddu       $t0, $zero, $zero (Delay Slot)
        SET_GPR_U64(ctx, 8, (uint64_t)GPR_U64(ctx, 0) + (uint64_t)GPR_U64(ctx, 0));
        ctx->in_delay_slot = false;
    ctx->pc = 0x295920u;
    if (runtime->hasFunction(0x295920u)) {
        ...
        if (ctx->pc == __entryPc) { ctx->pc = 0x28EBB4u; }
        if (ctx->pc != 0x28EBB4u) { return; }
    } else {
        const uint32_t __entryPc = ctx->pc;
...
label_28ebb8:
    ...
    ctx->pc = 0x28EBB4u;
    {
        ...
        ctx->pc = 0x28EBB8u;
        ctx->in_delay_slot = true; ctx->branch_pc = 0x28EBB4u;
            // 0x28ebb8: 0x102d  daddu       $v0, $zero, $zero (Delay Slot)
        SET_GPR_U64(ctx, 2, (uint64_t)GPR_U64(ctx, 0) + (uint64_t)GPR_U64(ctx, 0));
        ctx->in_delay_slot = false;
...
label_28ebc0:
    ...
    ctx->pc = 0x28EBBCu;
    {
        ...
        ctx->pc = jumpTarget;
        {
            ...
            const uint32_t __entryPc = ctx->pc;
            targetFn(rdram, ctx, runtime);
            if (ctx->pc == __entryPc) { ctx->pc = 0x28EBC4u; }
            if (ctx->pc != 0x28EBC4u) { return; }
        }
        }
    }
    ctx->pc = 0x28EBC4u;
