...
void sub_00174C70_0x174c70(uint8_t* rdram, R5900Context* ctx, PS2Runtime *runtime) {
    ...
    ctx->pc = 0x174CF0u;
    if (!runtime->dispatchGuestBranch(rdram, ctx, 0x174CF0u, 0x174CC0u, 0x174CC8u, PS2Runtime::GuestBranchKind::DirectCall, "JAL")) {
        return;
    }
    ctx->pc = 0x174CC8u;
label_174cc8:
...
label_174cd0:
    ...
    ctx->pc = 0x174F40u;
    if (!runtime->dispatchGuestBranch(rdram, ctx, 0x174F40u, 0x174CD0u, 0x174CD8u, PS2Runtime::GuestBranchKind::DirectCall, "JAL")) {
        return;
    }
    ctx->pc = 0x174CD8u;
label_174cd8:
...
label_174cdc:
    ...
    ctx->pc = 0x174cecu;
    // NOP
    if (ctx->pc == 0x174cecu) { ctx->pc = 0x174cf0u; }
}
