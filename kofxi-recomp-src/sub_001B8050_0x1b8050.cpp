...
label_1b8064:
    ...
    ctx->pc = 0x1B8068u;
    ctx->in_delay_slot = true; ctx->branch_pc = 0x1B8064u;
            // 0x1b8068: 0x200202d  daddu       $a0, $s0, $zero (Delay Slot)
        SET_GPR_U64(ctx, 4, (uint64_t)GPR_U64(ctx, 16) + (uint64_t)GPR_U64(ctx, 0));
        ctx->in_delay_slot = false;
    ctx->pc = 0x1B8080u;
    if (runtime->hasFunction(0x1B8080u)) {
        auto targetFn = runtime->lookupFunction(0x1B8080u);
        const uint32_t __entryPc = ctx->pc;
        targetFn(rdram, ctx, runtime);
        if (ctx->pc == __entryPc) { ctx->pc = 0x1B806Cu; }
        if (ctx->pc != 0x1B806Cu) { return; }
    } else {
        const uint32_t __entryPc = ctx->pc;
        sub_001B8080_0x1b8080(rdram, ctx, runtime);
        if (ctx->pc == __entryPc) { ctx->pc = 0x1B806Cu; }
        if (ctx->pc != 0x1B806Cu) { return; }
    }
    ctx->pc = 0x1B806Cu;
