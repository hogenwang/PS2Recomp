...
void sub_001307E0_0x1307e0(uint8_t* rdram, R5900Context* ctx, PS2Runtime *runtime) {
    ...
    ctx->pc = 0x1307ECu;
    {
        ...
        ctx->branch_pc = 0x1307ECu;
        // 0x1307f0: 0xa06555d0  sb          $a1, 0x55D0($v1) (Delay Slot)
        WRITE8(ADD32(GPR_U32(ctx, 3), 21968), (uint8_t)GPR_U32(ctx, 5));
        ctx->in_delay_slot = false;
        ctx->pc = jumpTarget;
        #if defined(PS2X_STRICT_RETURN_DIAGNOSTICS) && PS2X_STRICT_RETURN_DIAGNOSTICS
        (void)runtime->dispatchGuestBranch(rdram, ctx, jumpTarget, 0x1307ECu, 0u, PS2Runtime::GuestBranchKind::Return, "JR $ra");
        return;
        #else
        ctx->pc = jumpTarget;
        return;
    ...
    ctx->pc = 0x1307fcu;
    // NOP
    if (ctx->pc == 0x1307fcu) { ctx->pc = 0x130800u; }
}
