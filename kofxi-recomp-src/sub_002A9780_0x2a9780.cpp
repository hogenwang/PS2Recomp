...
void sub_002A9780_0x2a9780(uint8_t* rdram, R5900Context* ctx, PS2Runtime *runtime) {
    ...
    ctx->pc = 0x2A9784u;
    {
        ...
        ctx->branch_pc = 0x2A9784u;
        // 0x2a9788: 0x24426ab8  addiu       $v0, $v0, 0x6AB8 (Delay Slot)
        SET_GPR_S32(ctx, 2, (int32_t)ADD32(GPR_U32(ctx, 2), 27320));
        ctx->in_delay_slot = false;
        ctx->pc = jumpTarget;
        #if defined(PS2X_STRICT_RETURN_DIAGNOSTICS) && PS2X_STRICT_RETURN_DIAGNOSTICS
        (void)runtime->dispatchGuestBranch(rdram, ctx, jumpTarget, 0x2A9784u, 0u, PS2Runtime::GuestBranchKind::Return, "JR $ra");
        return;
        #else
        ctx->pc = jumpTarget;
        return;
    ...
    ctx->pc = 0x2a978cu;
    // NOP
    if (ctx->pc == 0x2a978cu) { ctx->pc = 0x2a9790u; }
}
