...
void sub_00303480_0x303480(uint8_t* rdram, R5900Context* ctx, PS2Runtime *runtime) {
    ...
    ctx->pc = 0x3034A0u;
    {
        ...
        (void)runtime->dispatchGuestBranch(rdram, ctx, jumpTarget, 0x3034A0u, 0u, PS2Runtime::GuestBranchKind::Return, "JR $ra");
        return;
        #else
        ctx->pc = jumpTarget;
        return;
    ...
    ctx->pc = 0x3034acu;
    // NOP
    if (ctx->pc == 0x3034acu) { ctx->pc = 0x3034b0u; }
}
