...
label_2f0db4:
    ...
    ctx->pc = 0x2F0DBCu;
    ctx->in_delay_slot = true;
    ctx->branch_pc = 0x2F0DB8u;
    // 0x2f0dbc: 0x302d  daddu       $a2, $zero, $zero (Delay Slot)
    SET_GPR_U64(ctx, 6, (uint64_t)GPR_U64(ctx, 0) + (uint64_t)GPR_U64(ctx, 0));
    ctx->in_delay_slot = false;
    ctx->pc = 0x2F0678u;
    if (!runtime->dispatchGuestBranch(rdram, ctx, 0x2F0678u, 0x2F0DB8u, 0x2F0DC0u, PS2Runtime::GuestBranchKind::DirectCall, "JAL")) {
        return;
    }
    ctx->pc = 0x2F0DC0u;
label_2f0dc0:
...
label_2f0dd4:
    ...
    ctx->pc = 0x2F0DD8u;
    {
        ...
        ctx->pc = 0x2F0DDCu;
        ctx->in_delay_slot = true;
        ctx->branch_pc = 0x2F0DD8u;
        // 0x2f0ddc: 0x27bd0010  addiu       $sp, $sp, 0x10 (Delay Slot)
        SET_GPR_S32(ctx, 29, (int32_t)ADD32(GPR_U32(ctx, 29), 16));
        ctx->in_delay_slot = false;
        ctx->pc = jumpTarget;
        #if defined(PS2X_STRICT_RETURN_DIAGNOSTICS) && PS2X_STRICT_RETURN_DIAGNOSTICS
        (void)runtime->dispatchGuestBranch(rdram, ctx, jumpTarget, 0x2F0DD8u, 0u, PS2Runtime::GuestBranchKind::Return, "JR $ra");
        return;
        #else
        ctx->pc = jumpTarget;
        return;
