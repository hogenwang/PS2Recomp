...
label_141a84:
    ...
    ctx->pc = 0x141B80u;
    {
        ...
        ctx->pc = 0x141B84u;
        ctx->in_delay_slot = true;
        ctx->branch_pc = 0x141B80u;
        // 0x141b84: 0xaca0008c  sw          $zero, 0x8C($a1) (Delay Slot)
        WRITE32(ADD32(GPR_U32(ctx, 5), 140), GPR_U32(ctx, 0));
        ctx->in_delay_slot = false;
        ctx->pc = jumpTarget;
        #if defined(PS2X_STRICT_RETURN_DIAGNOSTICS) && PS2X_STRICT_RETURN_DIAGNOSTICS
        (void)runtime->dispatchGuestBranch(rdram, ctx, jumpTarget, 0x141B80u, 0u, PS2Runtime::GuestBranchKind::Return, "JR $ra");
        return;
        #else
        ctx->pc = jumpTarget;
        return;
    ...
    ctx->pc = 0x141b8cu;
    // NOP
    if (ctx->pc == 0x141b8cu) { ctx->pc = 0x141b90u; }
}
