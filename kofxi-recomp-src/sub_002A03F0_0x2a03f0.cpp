...
label_2a042c:
    ...
    ctx->pc = 0x2A0428u;
    {
        ...
        ctx->pc = 0x2A042Cu;
        ctx->in_delay_slot = true; ctx->branch_pc = 0x2A0428u;
            // 0x2a042c: 0x24050004  addiu       $a1, $zero, 0x4 (Delay Slot)
        SET_GPR_S32(ctx, 5, (int32_t)ADD32(GPR_U32(ctx, 0), 4));
        ...
        ctx->pc = jumpTarget;
        {
            ...
            if (ctx->pc == __entryPc) { ctx->pc = 0x2A0430u; }
            if (ctx->pc != 0x2A0430u) { return; }
        }
        }
    }
    ctx->pc = 0x2A0430u;
...
label_2a0438:
    ...
    ctx->pc = 0x2A0434u;
    {
        ...
        ctx->pc = 0x2A0438u;
        ctx->in_delay_slot = true; ctx->branch_pc = 0x2A0434u;
            // 0x2a0438: 0x27bd0010  addiu       $sp, $sp, 0x10 (Delay Slot)
        SET_GPR_S32(ctx, 29, (int32_t)ADD32(GPR_U32(ctx, 29), 16));
        ctx->in_delay_slot = false;
        ctx->pc = jumpTarget;
