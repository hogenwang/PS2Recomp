...
label_230768:
    ...
    ctx->pc = 0x230768u;
    {
        ...
        (void)runtime->dispatchGuestBranch(rdram, ctx, jumpTarget, 0x230768u, 0u, PS2Runtime::GuestBranchKind::Return, "JR $ra");
        return;
        #else
        ctx->pc = jumpTarget;
        return;
...
label_2307b8:
    ...
    ctx->pc = 0x2307CCu;
    {
        ...
        ctx->pc = 0x2307D0u;
        ctx->in_delay_slot = true;
        ctx->branch_pc = 0x2307CCu;
        // 0x2307d0: 0xac800010  sw          $zero, 0x10($a0) (Delay Slot)
        WRITE32(ADD32(GPR_U32(ctx, 4), 16), GPR_U32(ctx, 0));
        ctx->in_delay_slot = false;
        ctx->pc = jumpTarget;
        #if defined(PS2X_STRICT_RETURN_DIAGNOSTICS) && PS2X_STRICT_RETURN_DIAGNOSTICS
        (void)runtime->dispatchGuestBranch(rdram, ctx, jumpTarget, 0x2307CCu, 0u, PS2Runtime::GuestBranchKind::Return, "JR $ra");
        return;
        #else
        ctx->pc = jumpTarget;
        return;
    ...
    ctx->pc = 0x2307d4u;
    // NOP
    if (ctx->pc == 0x2307d4u) { ctx->pc = 0x2307d8u; }
}
