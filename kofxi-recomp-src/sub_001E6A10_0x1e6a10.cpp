...
void sub_001E6A10_0x1e6a10(uint8_t* rdram, R5900Context* ctx, PS2Runtime *runtime) {
    ...
    ctx->pc = 0x1E6A30u;
    ctx->in_delay_slot = true; ctx->branch_pc = 0x1E6A2Cu;
    ...
    if (runtime->hasFunction(0x1CDFC8u)) {
        ...
        if (ctx->pc == __entryPc) { ctx->pc = 0x1E6A34u; }
        if (ctx->pc != 0x1E6A34u) { return; }
    } else {
        const uint32_t __entryPc = ctx->pc;
...
label_1e6a34:
    ...
    ctx->pc = 0x1E6A40u;
    {
        ...
        ctx->pc = 0x1E6A44u;
        ctx->in_delay_slot = true; ctx->branch_pc = 0x1E6A40u;
