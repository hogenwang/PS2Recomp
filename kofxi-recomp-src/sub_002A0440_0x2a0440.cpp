...
label_2a047c:
    ...
    ctx->pc = 0x2A0478u;
    {
        ...
        ctx->pc = 0x2A047Cu;
        ctx->in_delay_slot = true; ctx->branch_pc = 0x2A0478u;
            // 0x2a047c: 0x2405000a  addiu       $a1, $zero, 0xA (Delay Slot)
        SET_GPR_S32(ctx, 5, (int32_t)ADD32(GPR_U32(ctx, 0), 10));
        ...
        ctx->pc = jumpTarget;
        {
            ...
            if (ctx->pc == __entryPc) { ctx->pc = 0x2A0480u; }
            if (ctx->pc != 0x2A0480u) { return; }
        }
        }
    }
    ctx->pc = 0x2A0480u;
...
label_2a0488:
    ...
    ctx->pc = 0x2A0484u;
    {
        ...
        ctx->pc = 0x2A0488u;
        ctx->in_delay_slot = true; ctx->branch_pc = 0x2A0484u;
            // 0x2a0488: 0x27bd0010  addiu       $sp, $sp, 0x10 (Delay Slot)
        SET_GPR_S32(ctx, 29, (int32_t)ADD32(GPR_U32(ctx, 29), 16));
        ctx->in_delay_slot = false;
        ctx->pc = jumpTarget;
