...
void sub_00173B90_0x173b90(uint8_t* rdram, R5900Context* ctx, PS2Runtime *runtime) {
    ...
    ctx->pc = 0x173C10u;
    {
        ...
        ctx->branch_pc = 0x173C10u;
        // 0x173c14: 0xa4830026  sh          $v1, 0x26($a0) (Delay Slot)
        WRITE16(ADD32(GPR_U32(ctx, 4), 38), (uint16_t)GPR_U32(ctx, 3));
        ctx->in_delay_slot = false;
        ctx->pc = jumpTarget;
        #if defined(PS2X_STRICT_RETURN_DIAGNOSTICS) && PS2X_STRICT_RETURN_DIAGNOSTICS
        (void)runtime->dispatchGuestBranch(rdram, ctx, jumpTarget, 0x173C10u, 0u, PS2Runtime::GuestBranchKind::Return, "JR $ra");
        return;
        #else
        ctx->pc = jumpTarget;
        return;
    ...
    ctx->pc = 0x173c1cu;
    // NOP
    if (ctx->pc == 0x173c1cu) { ctx->pc = 0x173c20u; }
}
