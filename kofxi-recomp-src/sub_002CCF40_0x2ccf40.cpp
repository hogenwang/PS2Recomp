...
label_2cd0cc:
    ...
    ctx->pc = 0x2CD0CCu;
    {
        ...
        (void)runtime->dispatchGuestBranch(rdram, ctx, jumpTarget, 0x2CD0CCu, 0u, PS2Runtime::GuestBranchKind::Return, "JR $ra");
        return;
        #else
        ctx->pc = jumpTarget;
        return;
    ...
    ctx->pc = 0x2cd0d4u;
    // NOP
    if (ctx->pc == 0x2cd0d4u) { ctx->pc = 0x2cd0d8u; }
}
