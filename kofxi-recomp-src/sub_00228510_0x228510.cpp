...
void sub_00228510_0x228510(uint8_t* rdram, R5900Context* ctx, PS2Runtime *runtime) {
    ...
    ctx->branch_pc = 0x22851Cu;
    // 0x228520: 0xa0802d  daddu       $s0, $a1, $zero (Delay Slot)
    SET_GPR_U64(ctx, 16, (uint64_t)GPR_U64(ctx, 5) + (uint64_t)GPR_U64(ctx, 0));
    ctx->in_delay_slot = false;
    ctx->pc = 0x2284D8u;
    if (!runtime->dispatchGuestBranch(rdram, ctx, 0x2284D8u, 0x22851Cu, 0x228524u, PS2Runtime::GuestBranchKind::DirectCall, "JAL")) {
        return;
    }
    ctx->pc = 0x228524u;
label_228524:
...
label_22856c:
    ...
    ctx->pc = 0x228574u;
    {
        ...
        ctx->pc = 0x228578u;
        ctx->in_delay_slot = true;
        ctx->branch_pc = 0x228574u;
        // 0x228578: 0x27bd0020  addiu       $sp, $sp, 0x20 (Delay Slot)
        SET_GPR_S32(ctx, 29, (int32_t)ADD32(GPR_U32(ctx, 29), 32));
        ctx->in_delay_slot = false;
        ctx->pc = jumpTarget;
        #if defined(PS2X_STRICT_RETURN_DIAGNOSTICS) && PS2X_STRICT_RETURN_DIAGNOSTICS
        (void)runtime->dispatchGuestBranch(rdram, ctx, jumpTarget, 0x228574u, 0u, PS2Runtime::GuestBranchKind::Return, "JR $ra");
        return;
        #else
        ctx->pc = jumpTarget;
        return;
    ...
    ctx->pc = 0x22857cu;
    // NOP
    if (ctx->pc == 0x22857cu) { ctx->pc = 0x228580u; }
}
