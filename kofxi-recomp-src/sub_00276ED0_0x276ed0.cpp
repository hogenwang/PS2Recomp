...
void sub_00276ED0_0x276ed0(uint8_t* rdram, R5900Context* ctx, PS2Runtime *runtime) {
    ...
    ctx->pc = 0x276F08u;
    ctx->in_delay_slot = true; ctx->branch_pc = 0x276F04u;
            // 0x276f08: 0x1044021  addu        $t0, $t0, $a0 (Delay Slot)
        SET_GPR_S32(ctx, 8, (int32_t)ADD32(GPR_U32(ctx, 8), GPR_U32(ctx, 4)));
        ctx->in_delay_slot = false;
    ctx->pc = 0x2DAB00u;
    if (runtime->hasFunction(0x2DAB00u)) {
        auto targetFn = runtime->lookupFunction(0x2DAB00u);
        const uint32_t __entryPc = ctx->pc;
        targetFn(rdram, ctx, runtime);
        if (ctx->pc == __entryPc) { ctx->pc = 0x276F0Cu; }
        if (ctx->pc != 0x276F0Cu) { return; }
    } else {
        const uint32_t __entryPc = ctx->pc;
        sub_002DAB00_0x2dab00(rdram, ctx, runtime);
        if (ctx->pc == __entryPc) { ctx->pc = 0x276F0Cu; }
        if (ctx->pc != 0x276F0Cu) { return; }
    }
    ctx->pc = 0x276F0Cu;
