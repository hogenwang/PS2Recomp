...
void sub_0019C400_0x19c400(uint8_t* rdram, R5900Context* ctx, PS2Runtime *runtime) {
    ...
    ctx->pc = 0x19C428u;
    {
        ...
        if (!runtime->dispatchGuestBranch(rdram, ctx, jumpTarget, 0x19C428u, 0x0u, PS2Runtime::GuestBranchKind::IndirectJump, "JR")) {
            return;
        }
    }
    ctx->pc = 0x19C430u;
...
label_19c4bc:
    ...
    ctx->pc = 0x19C4BCu;
    {
        ...
        (void)runtime->dispatchGuestBranch(rdram, ctx, jumpTarget, 0x19C4BCu, 0u, PS2Runtime::GuestBranchKind::Return, "JR $ra");
        return;
        #else
        ctx->pc = jumpTarget;
        return;
    ...
    ctx->pc = 0x19c4ccu;
    // NOP
    if (ctx->pc == 0x19c4ccu) { ctx->pc = 0x19c4d0u; }
}
