...
void sub_001AEF20_0x1aef20(uint8_t* rdram, R5900Context* ctx, PS2Runtime *runtime) {
    ...
    ctx->pc = 0x1AEF48u;
    ctx->in_delay_slot = true; ctx->branch_pc = 0x1AEF44u;
    ...
    if (runtime->hasFunction(0x1B1AB0u)) {
        ...
        if (ctx->pc == __entryPc) { ctx->pc = 0x1AEF4Cu; }
        if (ctx->pc != 0x1AEF4Cu) { return; }
    } else {
        const uint32_t __entryPc = ctx->pc;
...
label_1aef4c:
    ...
    if (runtime->hasFunction(0x1AEF88u)) {
        ...
        const uint32_t __entryPc = ctx->pc;
        targetFn(rdram, ctx, runtime);
        if (ctx->pc == __entryPc) { ctx->pc = 0x1AEF60u; }
        if (ctx->pc != 0x1AEF60u) { return; }
    } else {
        const uint32_t __entryPc = ctx->pc;
