...
void sub_001F7750_0x1f7750(uint8_t* rdram, R5900Context* ctx, PS2Runtime *runtime) {
    ...
    ctx->pc = 0x1F7760u;
    ctx->in_delay_slot = true; ctx->branch_pc = 0x1F775Cu;
            // 0x1f7760: 0x80802d  daddu       $s0, $a0, $zero (Delay Slot)
        SET_GPR_U64(ctx, 16, (uint64_t)GPR_U64(ctx, 4) + (uint64_t)GPR_U64(ctx, 0));
        ctx->in_delay_slot = false;
    ctx->pc = 0x1F7778u;
    if (runtime->hasFunction(0x1F7778u)) {
        auto targetFn = runtime->lookupFunction(0x1F7778u);
        const uint32_t __entryPc = ctx->pc;
        targetFn(rdram, ctx, runtime);
        if (ctx->pc == __entryPc) { ctx->pc = 0x1F7764u; }
        if (ctx->pc != 0x1F7764u) { return; }
    } else {
        const uint32_t __entryPc = ctx->pc;
        sub_001F7778_0x1f7778(rdram, ctx, runtime);
        if (ctx->pc == __entryPc) { ctx->pc = 0x1F7764u; }
        if (ctx->pc != 0x1F7764u) { return; }
    }
    ctx->pc = 0x1F7764u;
label_1f7764:
    ...
    ctx->pc = 0x1FA240u;
    {
        auto targetFn = runtime->lookupFunction(0x1FA240u);
        const uint32_t __entryPc = ctx->pc;
        targetFn(rdram, ctx, runtime);
        return;
    }
