...
label_18bab0:
    ...
    ctx->pc = 0x18BAB0u;
    {
        ...
        (void)runtime->dispatchGuestBranch(rdram, ctx, jumpTarget, 0x18BAB0u, 0u, PS2Runtime::GuestBranchKind::Return, "JR $ra");
        return;
        #else
        ctx->pc = jumpTarget;
        return;
    ...
    ctx->pc = 0x18babcu;
    // NOP
    if (ctx->pc == 0x18babcu) { ctx->pc = 0x18bac0u; }
}
