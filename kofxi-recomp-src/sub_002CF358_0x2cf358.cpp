...
label_2cf380:
    ...
    ctx->pc = 0x2CF37Cu;
    {
        ...
        ctx->pc = 0x2CF380u;
        ctx->in_delay_slot = true; ctx->branch_pc = 0x2CF37Cu;
            // 0x2cf380: 0xa0902d  daddu       $s2, $a1, $zero (Delay Slot)
        SET_GPR_U64(ctx, 18, (uint64_t)GPR_U64(ctx, 5) + (uint64_t)GPR_U64(ctx, 0));
...
label_2cf388:
    ...
    if (runtime->hasFunction(0x2CF4B8u)) {
        ...
        if (ctx->pc == __entryPc) { ctx->pc = 0x2CF38Cu; }
        if (ctx->pc != 0x2CF38Cu) { return; }
    } else {
        const uint32_t __entryPc = ctx->pc;
...
label_2cf3a8:
    ...
    ctx->pc = 0x2CF3A4u;
    {
        ...
        ctx->pc = jumpTarget;
        {
            ...
            const uint32_t __entryPc = ctx->pc;
            targetFn(rdram, ctx, runtime);
            if (ctx->pc == __entryPc) { ctx->pc = 0x2CF3ACu; }
            if (ctx->pc != 0x2CF3ACu) { return; }
        }
        }
    }
    ctx->pc = 0x2CF3ACu;
